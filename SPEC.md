...
| `run_bash` | ❌ | Execute a bash command (optional `cwd`; optional `host` runs it over ssh; sudo via `SUDO_ASKPASS` with passwords cached per host). On Windows remote-only: `host` is required. |
...
**Windows tool surface (`run_powershell`):** Ported from Python Pengy (see its spec for the full rationale). `tool_definitions()` is `platform_tools(base_tool_definitions(), cfg!(windows), …)`: POSIX gets the base list unchanged; Windows gets `run_powershell` inserted before a remote-only `run_bash` (`host` required, and a local call is rejected by `windows_local_run_bash_error`), and the `download_file`/`fetch_url`/`glob` descriptions point at `run_powershell`. PowerShell is resolved once (`POWERSHELL`): `pwsh` on PATH, else `powershell`, else `%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe`; never `cmd.exe`. The description states the version (5.1 gets a list of syntax differences) and the elevation state (`IsUserAnAdmin` from shell32); there is no Windows elevation flow. The script is written to a `pengy-<pid>-<nanos>.ps1` temp file and run as `<ps> -NoLogo -NoProfile -NonInteractive -Command <POWERSHELL_PRELUDE>`; the prelude (byte-identical with the other editions, no double quotes) reads the file as UTF-8, compiles it with `[ScriptBlock]::Create`, turns off progress output and pwsh's ANSI rendering, sets UTF-8 output, exits 1 on a parse error, and otherwise exits with `$LASTEXITCODE` (1 if the script throws). Tool subprocesses on Windows get `CREATE_NO_WINDOW` (`hide_console`), and `terminate_process_group` runs `taskkill /T /F` by absolute path. `powershell_tests` covers the surface and, when `pwsh` is on PATH (it runs on Linux too), the live prelude.
